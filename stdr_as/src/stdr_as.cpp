...
dist = sqrt((goal_pose.position.x-current_pose.position.x)*(goal_pose.position.x-current_pose.position.x)+(goal_pose.position.y-current_pose.position.y)*(goal_pose.position.y-current_pose.position.y)); //true?
 if (dist < g_dist_tol) { //too small of a motion, so just set the heading from goal heading
   heading = convertPlanarQuat2Phi(goal_pose.orientation); 
    ...
    heading = atan2(goal_pose.position.y-current_pose.position.y,goal_pose.position.x-current_pose.position.x); //True?
 }

}


class StdrActionServer {
private:

    ros::NodeHandle nh_;
    actionlib::SimpleActionServer<stdr_as::stdrAction> as;
    stdr_as::stdrGoal goal;
    stdr_as::stdrResult result;
    stdr_as::stdrFeedback feedback;

public:
	StdrActionServer();
	~StdrActionServer(void) {

	}

void executeCb (const actionlib::SimpleActionServer<stdr_as::stdrAction>::GoalConstPtr& goal);
};


StdrActionServer::StdrActionServer():
	   as(nh_, "stdr_as", boost::bind(&StdrActionServer::executeCb, this, _1),false) {
	   	as.start();
	   	}


void StdrActionServer::executeCb(const actionlib::SimpleActionServer<stdr_as::stdrAction>::GoalConstPtr& goal) {
...
int main(int argc, char **argv) {
  ...
  ros::init(argc, argv, "stdr_as");
  ros::NodeHandle n;
  StdrActionServer as_object;  
ros::Subscriber alarm_subscriber = n.subscribe("left_lidar_alarm",1, alarmCb);
ros::Subscriber alarm_subscriber2 = n.subscribe("right_lidar_alarm",1, alarmCb); 
