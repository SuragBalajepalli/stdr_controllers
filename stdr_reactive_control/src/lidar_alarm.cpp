...
int mid_index_= -1; // NOT real; callback will have to find this
double angle_min_=0.0;
double angle_max_=0.0;
double angle_increment_=0.0;
double range_min_ = 0.0;
double range_max_ = 0.0;
bool left_laser_alarm_=false;
bool front_laser_alarm_=false;
bool right_laser_alarm_=false;
...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
   ...
   for (int i=mid_index-50; i<mid_index+50;i++) {
   ...
   left_lidar_alarm_msg.data = right_laser_alarm;
...
int main(int argc, char **argv) {
    ...
    left_lidar_alarm_publisher_ = pub; // let's make this global, so callback can use it
